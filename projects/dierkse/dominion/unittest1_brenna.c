/* -----------------------------------------------------------------------
 * Unit test for Team Member's BaronEffect()
 * Emily Dierks
 * CS362, Summer 2019
 *  Compile: gcc -std=c99 -lm unittest1_brenna.c dominion_brenna.c rngs.c -o unittest1bren
 * -----------------------------------------------------------------------
 */
...
//Test choice1 = 0, player has estate card then can get estate card instead of gaining 4 coins
	printf("\n\nTest choice1=0 and player has estate card\n");
	setUpCards(&G, &testG, 1);
	baronEffect(1, 0, &testG);

	//Check if only discard Baron & don't put estate card into discard
	printf("ESTATE CARD SUPPLY: Expected value: %d, Result: %d. ", G.supplyCount[estate]-1, testG.supplyCount[estate]);
	if(G.supplyCount[estate]-1 == testG.supplyCount[estate]){ printf("SUCCESS\n");}
	else{ printf("FAIL\n");}
	printf("BARON CARD & NEW ESTATE IN DISCARD: Expected value: %d, Result: %d. ", G.discardCount[G.whoseTurn]+2, testG.discardCount[testG.whoseTurn]);
	if(G.discardCount[G.whoseTurn]+2 == testG.discardCount[testG.whoseTurn]){ printf("SUCCESS\n");}
	else{ printf("FAIL\n");}
